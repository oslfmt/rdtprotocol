...
/**
 * FULLY FUNCTIONAL RDT STOP-AND-WAIT PROTOCOL
 * ONLY HANDLES BIT CORRUPTION
 * NOT PACKET LOSS
 */
...
struct Receiver {
  ...
  // used for testing
  int num_delivered;
  struct msg msgs_delivered[NUMPKT];
} B;
  ...
  const struct pkt packet;
{
  ...
  strncpy(B.msgs_delivered[B.num_delivered].data, packet.payload, 20);
  B.num_delivered++;
}
  ...
  struct pkt packet;
{
  ...
  if (result == -1 && packet.seqnum == B.expectedseq) {
    ...
    printf("Sent to layer 5 and sending ACK\n");
...
void check_rdt()
{
  struct msg* i = B.msgs_delivered;
  while (i != NULL) {
    for (unsigned int j = 0; j < sizeof(i->data); j++) {
      printf(i->data[j] + " ");
    }
    printf("\n");
    i++;
